...
public final class FlowerAgentAiModelGateway implements AiModelGateway {
    ...
    private final AgentEventSink agentEvents;
    ...
    public AiModelCall submit(AiModelRequest request) {
        ...
        int attempt = task.agentAttempts().size() + 1;
        Instant now = clock.instant();
        Map<String, Object> metadata = new LinkedHashMap<>();
        metadata.put(IssueRefundTool.TASK_ID_METADATA, taskId);
        metadata.put("sample.orderId", task.orderId());
        ...
        AgentSpec spec = new AgentSpec(
                ...
                metadata);
        AgentRecipe recipe = AgentFlows.react(spec)
        ...
        AgentThread thread = new AgentThread(
                ...
                now,
                Map.of(IssueRefundTool.TASK_ID_METADATA, taskId, "sample.orderId", task.orderId()));
        AgentRunFlow agentRun = recipe.createRun(
                thread,
                AgentMessage.user(userPrompt(request.prompt()), now));
        String harnessRunId = task.harnessFlow().context().runId().value();
        correlations.register(agentRun.run().runId(), taskId, harnessRunId);
        tasks.attachAgent(taskId, agentRun);
    ...
    private static String systemPrompt(RenderedPrompt prompt) {
        String joined = prompt.messages().stream()
                ...
                .orElse("");
        return "Use tools for every operational fact. Never invent eligibility or action results.\n\n" + joined;
    }
