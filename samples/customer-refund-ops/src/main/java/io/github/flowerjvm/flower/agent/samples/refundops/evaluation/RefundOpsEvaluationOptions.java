...
final class RefundOpsEvaluationOptions {
    ...
    private RefundOpsEvaluationOptions(
            boolean help,
            boolean scripted,
            String experimentId,
    ...
    static RefundOpsEvaluationOptions parse(String[] args) {
        boolean help = false;
        boolean scripted = true;
        String experimentId = null;
        String candidateVersion = null;
        String baselineExperimentId = null;
        Path observationFile = Paths.get("build/evaluation/flower-observations.jsonl");
        ...
        if (args != null) {
            for (String argument : args) {
                ...
                } else if ("observation-file".equals(name)) {
                    observationFile = Paths.get(required(name, value));
        ...
        return new RefundOpsEvaluationOptions(
                help,
                scripted,
                experimentId == null ? "customer-refund-ops-" + mode + "-v1" : experimentId,
                candidateVersion == null ? mode + "-v1" : candidateVersion,
                baselineExperimentId,
                absolute(observationFile),
                absolute(resultFile),
                absolute(feedbackFile),
                timeout);
    }

    static String helpText() {
        return "RefundOps evaluation options:\n"
                ...
                + "  --observation-file=<path>          Linked Flower observation JSONL\n"
                + "  --result-file=<path>               Evaluation result JSONL\n"
                + "  --feedback-file=<path>             Local feedback JSONL for Studio\n"
